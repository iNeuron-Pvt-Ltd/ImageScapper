...
def app():
    ...
    platform = st.sidebar.selectbox("Select the platform you want to scrape",
                                    ('Instagram', 'Naukari', 'Google', 'Baidu'))
    ...
    try:
        if platform == 'Naukari':
            ...
            df = pd.DataFrame(
                columns=['Job Title', 'Job URL', 'Company Title', 'Company URL', 'Ratings', 'Reviews', 'Experience','Salary', 'Location', 'Job_Post_History'])
            ...
            for job_elem in job_elems:
                ...
                df = df.append(
                    {'Job Title': job_title,'Job URL': job_URL, 'Company Title': company_title, 'Company URL': company_URL, 'Ratings': Ratings, 'Reviews': reviews,
